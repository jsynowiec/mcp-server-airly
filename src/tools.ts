...
function missingCoordsError() {
  return {
    content: [
      {
        type: "text" as const,
        text: "Latitude and longitude are required. Provide coordinates or configure default coordinates for this server.",
      },
    ],
    isError: true,
  };
...
function formatMeasurement(
  ...
  if (showCurrent) {
    lines.push(
      `Measurement period: ${current.fromDateTime} — ${current.tillDateTime}`,
    );
    lines.push("");
    ...
    if (current.indexes.length > 0) {
      for (const idx of current.indexes) {
        lines.push(
          `Air Quality Index (${idx.name}): ${idx.value} — ${idx.level}`,
        );
        if (idx.description || idx.advice) {
          const advisory = [idx.description, idx.advice]
            .filter(Boolean)
            .join(" ");
          lines.push(
            `  Advisory (human-readable message from Airly): ${advisory}`,
          );
        }
      }
      lines.push("");
    ...
    if (current.standards.length > 0) {
      ...
      for (const std of current.standards) {
        lines.push(
          `  ${std.pollutant} at ${std.percent}% of ${std.name} limit (${std.limit} µg/m³)`,
        );
      }
      lines.push("");
    }
  ...
  if (showHistory && measurement.history.length > 0) {
    ...
    for (const period of measurement.history) {
      lines.push(
        `  ${period.fromDateTime} — ${period.tillDateTime}: ${formatValuesCompact(period)}`,
      );
    }
    lines.push("");
  }
...
function formatInstallation(installation: Installation): string {
  ...
  lines.push(
    `  Address: ${[addr.city, addr.street, addr.number].filter(Boolean).join(", ")}`,
  );
  lines.push(
    `  Coordinates: ${installation.location.latitude}, ${installation.location.longitude}`,
  );
...
export function registerTools(
  ...
  server.registerTool(
    ...
    async (args) => {
      ...
      try {
        const measurement = await client.getMeasurementPoint(
          coords.latitude,
          coords.longitude,
          {
        ...
        return {
          content: [
            {
              type: "text",
              text: formatMeasurement(measurement, args.include),
            },
          ],
        };
      } catch (error) {
  ...
  server.registerTool(
    ...
    async (args) => {
      ...
      try {
        const installations = await client.getNearestInstallations(
          coords.latitude,
          coords.longitude,
          {
        ...
        return {
          content: [
            { type: "text", text: formatInstallationList(installations) },
          ],
        };
      } catch (error) {
        return handleApiError(error);
  ...
  server.registerTool(
    ...
    async (args) => {
      try {
        const measurement = await client.getMeasurementInstallation(
          args.installationId,
          {
            indexType: args.indexType,
            indexPollutant: args.indexPollutant,
  ...
  server.registerTool(
    ...
    async (args) => {
      try {
        ...
        return {
          content: [{ type: "text", text: formatInstallation(installation) }],
        };
      } catch (error) {
        return handleApiError(error);
      }
